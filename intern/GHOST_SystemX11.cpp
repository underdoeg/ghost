...
processEvents(
	...
	bool anyProcessedGlobal = false;
	//hook in to support multiple displays
	std::vector<Display*>::iterator it = displays.begin();
	while(it != displays.end()) {
		...
		do {
			...
			if(anyProcessed)
				anyProcessedGlobal = true;
#endif
		} while (waitForEvent && !anyProcessed);
	}
	return anyProcessedGlobal;
}
void
...
void GHOST_SystemX11::setDisplay(Display* d) {
	//somehow I didn't manage to get std::find working...
	bool found = false;
